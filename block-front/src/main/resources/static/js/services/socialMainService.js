...
define(['jquery','app' ], function ($,app) {
    ...
    app.factory('socialMainService', ['$http','$q','$resource' , function($http,$q,$resource ) { 
    	
    	function  searchResult(content,field,page){
    		//alert("content="+content+",field="+field+",page="+page);
    		var url = _projPath+ 'social/search?';
    	...
    	function  searchFacetResult(content,field){
    		//alert("content="+content+",field="+field+",page="+page);
    		var url = _projPath+ 'social/facet?';
			content = encodeURIComponent(content);
			url = url+"condition="+content;
			//url = encodeURIComponent(url);
			if(field!=undefined){
    	...
    	function  getSearchHis(){
    		//alert("content="+content+",field="+field+",page="+page);
    		var url = _projPath+ 'social/searchhis?';
			return $resource(url, {});
 		}
