define(['app','suggestMainService'], function(app,suggestMainService){
      
   return app.controller('suggestMainCtrl', ['$scope','$rootScope','$http','suggestMainService', function ($scope,$rootScope,$http,suggestMainService) {
...
//			}, 2000);
	    ...
	    $scope.getHis = function(){
	    	suggestMainService.getSearchHis().get({},function(resp){
		...
		$scope.searchResult = function(content,field,num) {
			 ...
			 }else if(field=='userinfo.userName'){
			...
			suggestMainService.searchResult(content,field,num).get({},function(resp)
			{
			...
			suggestMainService.searchFacetResult(content,field).get({},function(resp)
			{
		...
		$scope.getkeywords  =function(_result){
			...
			suggestMainService.keywords(_result._id).get({},function(resp){
		...
		$scope.getorganizer = function(_result){
			suggestMainService.getorganizer(_result._id).get({},function(resp){
